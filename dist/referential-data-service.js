...
export class Plan {
  ...
  execute(items) {
    ...
    let promises = _.reduce(this.steps, (previous, step) => previous.bind(step).then(step.execute), Promise.resolve(planContext));
...
export class PlanStep {
  ...
  execute(planContext) {
    ...
    return Promise.resolve(stepContext).then(sc => {
      // Get the ids
...
class ReferentialDataService {
  ...
  loadReferential(pagedItems, type, properties) {
    ...
    return plan.execute(pagedItems.items).then(planContext => pagedItems);
  }
